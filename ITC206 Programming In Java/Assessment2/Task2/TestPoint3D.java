...
public class TestPoint3D {
    ...
    // Method that finds the two furthest points out of an array of points Point3D objects
    public static void max(Point3D[] points) {
        ...
        for (int i = 0; i < points.length; i++) { 
            for (int j = i+1; j < points.length; j++) { // Starting the second loop at i+1 to avoid iterating and calculating the same coordinates more than once
                ...
                // Update the max distance and corresponding points if a greater distance is found:
    ...
    // Method that finds the two closest points out of an array of points Point3D objects
    public static void min(Point3D[] points) {
        ...
        for (int i = 0; i < points.length; i++) { 
            for (int j = i+1; j < points.length; j++) { // Starting the second loop at i+1 to avoid iterating and calculating the same coordinates more than once
                ...
                // Update the min distance and corresponding points if a smaller distance is found:
